...
const { rows } = await c.database.query({ token,
  sql: `SELECT id, version, status, data FROM lk_profiles ORDER BY version`, nodeId: 'rocketride_sql_1' });
for (const r of rows) {
  let verdict = typeof r.data;
  if (typeof r.data === 'string') {
    try { JSON.parse(r.data); verdict = 'string(valid JSON)'; }
    catch { verdict = 'string(PYTHON TEXT)'; }
  }
  console.log(`id=${r.id} v${r.version} ${r.status} -> ${verdict}`);
}
process.exit(0);
