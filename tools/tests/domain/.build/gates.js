...
exports.HN_LOCK_REJECTION_WHY = exports.HN_LOCK_SECONDS = exports.ASSET_LIMITS = exports.THREAD_PAT = void 0;
exports.gateSignals = gateSignals;
exports.gateAsset = gateAsset;
exports.hnLockCheck = hnLockCheck;
const py_1 = require("./py");
/** rr.THREAD_PAT — what counts as a real discussion thread. */
exports.THREAD_PAT = new RegExp("(reddit\\.com/r/.+/comments/|news\\.ycombinator\\.com/item|" +
...
function gateSignals(signals, ownUrls) {
    const domains = [];
    for (const u of ownUrls) {
        const su = (0, py_1.pyStr)(u);
        ...
        if (m) {
            ...
            if (GENERIC_HOSTS.has(host)) {
                ...
                if (segs.length)
                    domains.push(`${host}/${segs.join("/")}`.toLowerCase());
            }
            else {
                domains.push(host);
            }
    ...
    for (const s of signals) {
        const url = (0, py_1.pyStr)((0, py_1.pyGet)(s, "url", ""));
        ...
        else if (!exports.THREAD_PAT.test(url)) {
 ...
 */
function gateAsset(assetType, data) {
    const raw = (0, py_1.pyGet)(data, "warnings", null);
    const warnings = (0, py_1.pyTruthy)(raw) ? (0, py_1.pyList)(raw) : [];
    const limit = Object.prototype.hasOwnProperty.call(exports.ASSET_LIMITS, assetType)
        ? exports.ASSET_LIMITS[assetType]
        : undefined;
    if (limit) {
        const [field, maxLen] = limit;
        if ((0, py_1.pyLen)((0, py_1.pyStr)((0, py_1.pyGet)(data, field, ""))) > maxLen) {
            warnings.push(`${field} exceeds ${maxLen} chars — trim before publishing`);
        }
    }
    if (assetType === "show_hn" && !(0, py_1.pyStr)((0, py_1.pyGet)(data, "title", "")).startsWith("Show HN:")) {
        warnings.push("title must start with 'Show HN:'");
    }
    if (assetType === "reddit_post" && (0, py_1.pyStr)((0, py_1.pyGet)(data, "title", "")).startsWith("Show HN")) {
...
exports.HN_LOCK_SECONDS = 14 * 86400;
/** Rejection text used by rescore_signals when the HN lock trips. */
exports.HN_LOCK_REJECTION_WHY = "HN thread locked (older than 14 days) — cannot reply";
/**
 ...
 */
function hnLockCheck(url, createdEpoch, nowEpochSeconds) {
    if ((0, py_1.pyTruthy)(createdEpoch) && url.includes("news.ycombinator.com") &&
        nowEpochSeconds - createdEpoch > exports.HN_LOCK_SECONDS) {
        return { verdict: "rejected", why: exports.HN_LOCK_REJECTION_WHY };
    }
    return null;
}
