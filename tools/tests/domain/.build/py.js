...
exports.pyTruthy = pyTruthy;
exports.pyGet = pyGet;
exports.pySlice = pySlice;
exports.pyLen = pyLen;
exports.pyNumStr = pyNumStr;
exports.pyStrRepr = pyStrRepr;
...
function pyTruthy(v) {
    ...
    if (v === true)
        return true;
    if (typeof v === "number")
        return v !== 0; // NaN !== 0 → truthy, matching bool(float('nan'))
    if (typeof v === "string")
        return v.length > 0;
    if (Array.isArray(v))
        return v.length > 0;
...
function jsonStringEscape(s, ensureAscii) {
    let out = '"';
 ...
 */
function pyJsonDumps(value, opts) {
    ...
    const dump = (v) => {
        ...
        if (typeof v === "string")
            return jsonStringEscape(v, ensureAscii);
        if (Array.isArray(v))
