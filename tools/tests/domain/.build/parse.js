...
exports.pyLiteralEval = pyLiteralEval;
...
function pyLiteralEval(src) {
    ...
    const oneString = () => {
        ...
        while (i < src.length) {
            ...
            if (c === "\n" && !triple)
                return err("EOL inside string literal");
            out += c;
            i++;
        }
    ...
    const dictKey = (k) => (typeof k === "string" ? k : (0, py_1.pyStr)(k));
    const dictOrSet = () => {
        i++; // consume '{'
        ws();
        if (src[i] === "}") {
...
function parseJsonLoose(raw) {
    ...
    const text = (0, py_1.pyStr)(raw).trim();
    const start = text.indexOf("{");
    const end = text.lastIndexOf("}");
    if (start === -1 || end === -1) {
        throw new Error(`no JSON object in answer: ${(0, py_1.pyRepr)((0, py_1.pySlice)(text, 300))}`);
    }
