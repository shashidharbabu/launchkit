...
for (const r of rows) {
  if (typeof r.data === 'string') {
    ...
    const res = await q(`UPDATE lk_profiles SET data = $2::jsonb WHERE id = $1`,
      [r.id, JSON.stringify(parsed)]);
    console.log('updated', r.id, 'affected:', res.affected_rows);
  }
}
const after = await q(`SELECT version, data FROM lk_profiles ORDER BY version DESC LIMIT 1`);
const d = after.rows[0].data;
console.log('after: type =', typeof d, '| keys =', typeof d === 'object' ? Object.keys(d).join(',') : String(d).slice(0, 50));
process.exit(0);
