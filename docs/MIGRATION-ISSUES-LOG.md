...
| J6 | "No signals, but this app has demand; get the right demand places." | The scan searched narrow `site:reddit.com/r/...` product-category queries; HN threads were stale; StackOverflow returned nothing; X, LinkedIn, dev.to, Indie Hackers and forums were never searched; the question carried only `icp.pain`. | Pipe: QUERY STRATEGY (8 to 12 pain phrasings in the ICP's words, open-web first, Reddit broadly, niche subreddits last). Question: `PAIN_HINTS` from one_liner, description, use_cases, differentiators, category, target_user. Gate: `THREAD_PAT` accepts X, LinkedIn, dev.to, Indie Hackers, Quora posts. Pipe id rotated. |
