...
| I5 | Model output shown in the app (navigator replies, signal replies, listing copy, campaign ideas) could still carry em dashes; only Social Launch was sanitised. | `runner.askOnce` now runs `sanitizeDraft` on every pipe result before the app sees it (the asset card still reports the count via `punctuationFixed`). The navigator pipe has no agent node; its rules live in `data/navigator.prompt.json`, which now carries the PUNCTUATION rule. `tools/probe-navigator.mjs` calls the pipe directly and so bypasses the sanitiser; a dash in its output is a model-tone signal, not an app defect. |
| H6 | At 16:36:44, the minute registry version 20 registered, `launchkit/` was rewritten: a copy of the repo root (own `.git`, `pnpm-workspace.yaml`, `apps/`, `tools/`, `docs/`) replaced it and the original contents (backend, frontend drives, evals fixtures, docs) were moved into a nested `launchkit/launchkit/`. 509 tracked files went missing; the preview dev server died around the same time. Nothing in `tools/` does this; it matches the extension materialising an app workspace named after the app slug. | Recovery: `rm -rf launchkit/.git`, move any ignored files back (`frontend/node_modules`, `backend/.venv`, `.env`), `git checkout -- launchkit launchkit-design-system`, `git clean -fd -e .git.disabled launchkit`, then re-apply uncommitted pipe source edits from the workspace copies and `gen-pipes`. Rule: **after every deploy run `git status --short | grep -c '^ D'`; anything but 0 means the tree was rewritten.** Never `git add -A` while a nested `.git` exists under `launchkit/`. Preview restart: `cd apps/launchkit && npx rsbuild dev -c rsbuild.preview.mts` (port 3400). |

## J. Owner test of the hack-judge launch, 2026-09-03 (evening)

| # | Issue (owner's words) | Cause | Fix |
|---|------|-------|-----|
| J1 | "After profile approve it should automatically go to next step; after Brand there's no option to go next; other steps had no way to say done and go next." | Stages were islands: the only navigation was the rail, and nothing said what the plan still needed from each stage. | `StageNext` footer under every stage: the stage's state (posts approved, venues selected), what the plan needs, and one "Next: <stage>" button; approving the profile jumps to Brand. Plan's "not ready" already links to the missing gate. |
| J2 | "What is the difference between campaign and social launch, both are doing the same thing." | Campaigns shipped sample copy (headline, body, CTA), so they read as posts. | Reframed as **campaign angles** (the story and where to tell it); the sample copy is labelled an illustration. The builder chooses angles ("Use this angle"); Social Launch shows the chosen angle and the draft prompt carries `CAMPAIGN_ANGLE`. |
| J3 | "Which Claude key/model? Should be Sonnet with context of previous runs + rulebooks." | Every pipe already ran `claude-sonnet-4-6` through the owner's `ROCKETRIDE_ANTHROPIC_KEY`; the draft prompt had profile, brand DNA, rulebook and feedback, but not the previous draft, pricing/listing or the chosen angle. | `buildAssetQuestion` extras: `PREVIOUS_DRAFT` (last version of that post), `COMMERCIAL` (approved pricing + listing), `CAMPAIGN_ANGLE`. |
| J4 | "The app URL should be the actual URL instead of a placeholder." | Drafts store `{APP_URL}` so the plan can substitute per-venue tracked links; the card showed the raw placeholder. | The card fills it from the launch's app URL (then site URL) for display, copy and share (`fillDeep`); storage keeps the placeholder for tracked links. |
| J5 | "Venues 2 and 3 are absolutely wrong (awesome-lists). Where did this go wrong?" | The targets pipe told the model to include "GitHub awesome-lists for the app's niche", said "niche beats big", and nothing deterministic re-ranked or capped a listing; a README `Resources.md` was accepted as a venue with impact "high". | Pipe: awesome-lists only with a PR process, ICP watering holes as a venue class, ranking guardrails (listings rank 6+, impact capped). Code: `gateTargets` drops repository files, sinks awesome-lists and GitHub listings below launch venues with impact "low", caps directories at "medium", renumbers ranks; dropped items saved as `targets_meta`. Node tests. Pipe id rotated. |
| J6 | "No signals, but this app has demand; get the right demand places." | The scan searched narrow `site:reddit.com/r/...` product-category queries; HN threads were stale; StackOverflow returned nothing; X, LinkedIn, dev.to, Indie Hackers and forums were never searched; the question carried only `icp.pain`. | Pipe, second iteration after a real scan still returned Reddit/HN/GitHub-only queries and lost its own Reddit findings before compiling: PROCESS rewritten as five mandatory passes with the open web FIRST (Exa, no site filter: X, LinkedIn, dev.to, Indie Hackers, forums), Reddit broadly second, niche subreddits and HN later, GitHub issues last and weighted lowest; a running CANDIDATES ledger after every tool result (the agent had reported "Reddit signals were not retained in memory context at compile time"); Exa payload trimmed (no full page text, 8 results) so 13 searches no longer overflow the context. Question: `PAIN_HINTS` from one_liner, description, use_cases, differentiators, category, target_user. Gate: `THREAD_PAT` accepts X, LinkedIn, dev.to, Indie Hackers, Quora posts. Pipe id rotated. |
| J7 | "Tick the venues you'll actually dofive right venues" (glued words). | The dash rewrite dropped a line-leading dash inside JSX, and JSX trims leading whitespace on the next line. | Fixed both sites (Targets, Social Launch lead text); the checked rule is now "no dash removal without a separator". |
| J8 | Store pipe id rotation. | The team workspace table lives in the store pipe's managed SQL node; its database may be scoped to the pipe's project id. | **Never rotate `lk_store`'s id.** Config changes to its unused LLM node do not need a restart. Pinned at `5c2f81d4…`. |
