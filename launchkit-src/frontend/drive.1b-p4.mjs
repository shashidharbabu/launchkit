...
try{
  ...
  await page.evaluate((raw)=>{const st=JSON.parse(raw);const lk=st.launchkit;lk.runs=(lk.runs||[]).filter(r=>!['pricing','listing'].includes(r.kind));lk.commercial_results=(lk.commercial_results||[]).filter(c=>!['pricing','listing'].includes(c.kind));for(const r of lk.runs){if(r.status==='running'||r.status==='queued'){r.status='error';r.error='seed: interrupted';}}localStorage.setItem('lk-preview-appstate',JSON.stringify(st));localStorage.removeItem('lk-nav');},raw);
  ...
  if(await goStage(/03\s*COMMERCIAL/i,'commercial')){await runAction('commercial',/draft pricing/i,'Draft pricing & listing');await waitIdle();log('RESULTS',JSON.stringify(await kinds()));await page.screenshot({path:`${OUT}/p4-commercial.png`});}
  if(await goStage(/06\s*SIGNALS/i,'signals')){await runAction('signals',/scan for live demand/i,'Scan for live demand',420);await page.waitForTimeout(3000);const a=await appState();const sg=a.signals||[];log('SIGNALS',JSON.stringify({count:sg.length,platforms:[...new Set(sg.map(s=>(s.data?.platform)||s.platform))].slice(0,5)}));await clickBtn('signals',/keep/i,'keep',8000);log('SIGNALS_AFTER_KEEP',JSON.stringify({statuses:[...new Set(((await appState()).signals||[]).map(s=>s.status))]}));await page.screenshot({path:`${OUT}/p4-signals.png`});}
  ...
  writeFileSync(`${OUT}/1b-appstate-p4.json`,await page.evaluate(()=>localStorage.getItem('lk-preview-appstate')||'{}'));
  log('RUNS_FINAL',JSON.stringify(await kinds()));
}catch(e){log('DRIVE ERROR',String(e).slice(0,220));}
log('pageerrors',errs.length,errs[0]||''); log('1B_P4_DONE'); await b.close(); process.exit(0);
