...
export type PlatformKey = 'github' | 'reddit' | 'hackernews' | 'producthunt' | 'x' | 'substack';

/** `scale` optically balances the solid marks (HN's square, Substack's bars)
 *  against the outline ones so no single logo shouts in the row. */
const MARKS: Record<PlatformKey, { title: string; path: string; scale?: number }> = {
  github: {
    title: 'GitHub',
...
/** The venues Launch Kit drafts a native post for. */
export const LAUNCH_PLATFORMS: PlatformKey[] = [
  'reddit',
  'hackernews',
  'producthunt',
  'x',
...
export function platformTitle(name: PlatformKey) {
  return MARKS[name].title;
}
...
export function PlatformIcon({
  ...
  const box = Math.round(size * (mark.scale ?? 1));
  return (
    <svg
      viewBox="0 0 24 24"
      width={box}
