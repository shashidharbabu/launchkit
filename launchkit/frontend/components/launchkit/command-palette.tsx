...
import { useProjectMaybe } from '@/components/launchkit/project-provider';
...
const APP_PAGES = [
  ...
  { href: '/launches/new', label: 'New launch' },
...
export function CommandPalette() {
  ...
  React.useEffect(() => {
    if (open) api.listProjects().then(setProjects).catch(() => {});
  }, [open]);
        ...
        <Command label="Command palette" className="w-full">
          ...
          <Command.List className="max-h-80 overflow-y-auto p-2">
            ...
            <Command.Group heading="Go to" className={GROUP_CLS}>
              {APP_PAGES.map((p) => (
                <Command.Item key={p.href} onSelect={() => run(() => router.push(p.href))} className={ITEM_CLS}>
                  {p.label}
                </Command.Item>
              ))}
            </Command.Group>
            ...
            {ctx?.project && id && (
              <Command.Group heading="Stages" className={GROUP_CLS}>
                {STAGES.map((s) => (
                  <Command.Item
                    key={s.slug}
                    onSelect={() => run(() => router.push(`/p/${id}/${s.slug}`))}
                    className={ITEM_CLS}
                  >
            ...
            <Command.Group heading="Your launches" className={GROUP_CLS}>
              {projects.map((p) => (
                <Command.Item
                  key={p.id}
                  onSelect={() => run(() => router.push(`/p/${p.id}/profile`))}
                  className={ITEM_CLS}
                >
                  {p.name}
                  {p.id === id && (
